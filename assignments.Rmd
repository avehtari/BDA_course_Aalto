...
- If you are a student in this course, you are allowed to discuss assignments with your friends, but it is not allowed to copy solutions directly from other students or from the internet.
...
Assignments are linked from [the schedule](Aalto2026.html#Schedule_2026)
...
schedule](Aalto2026.html#Schedule_2026).

-   The assignments are weighted as 5%, 5%, 10%, 10%, 15%, 20%, 15%, 10%, 10% (total 100%)
-   The assignments are introduced on Mondays.
-   The assignment answers are submitted via MyCourses Quiz questions with links in MyCourses.
-   Each assignment also has a Quarto template with some R code, but you don't need to submit any report. Just
    answer the questions in the MyCourses Quiz.
-   There are chat channels **#assignment1** etc. You can ask questions
    ...
    questions that are likely to have relatively simple answers.
...
-   We highly recommend submitting all assignments by Friday before 3 pm so
    that you can get TA help before submission. As the course has
    students who work weekdays (e.g., FiTech students), late
    submission until Sunday night is allowed, but we can't provide
    support during the weekends. Sisu and MyCourses list Friday TA sessions,
    which are not organized unless otherwise announced.
-   Your Quiz answers are auto-submitted by MyCourses at the deadline
    time, even if you have answered only some of the questions.
...
8 am of the hand-in period of the corresponding assignment week (See the
deadlines below, e.g. the first assignment is not guaranteed to be up to
date until `r sdate("Assignment opens (08:00)", "Week1")` at 8 am).

The assignments are mostly solved using a computer (R and Stan). Related
demos for each assignment are available in the course web pages (links
in the Materials section). See [TA sessions](#TA_sessions) for getting help.
...
page in
[MyCourses](https://mycourses.aalto.fi/course/view.php?id=51818). Course
...
-   There are chat channels **#assignment1**, etc., where you can ask questions
    about the assignments. Other students and TAs can answer these.
    There is no guaranteed response time. These channels are best for
    questions that are likely to have relatively simple answers and thus
...
-   There are [TA sessions](#TA_sessions) for getting one-on-one help.
    ...
    questions are answered during the TA session time (if there are too
    many questions, they may be answered in the chat or next TA
    session).

There are two TA sessions each week; please see [the course
schedule](Aalto2026.html#Schedule_2026) for more details on the meeting
...
what the problem is that you would need help with (see below).
...
-   Login with your Aalto account to the Zulip course chat (link in
    [MyCourses](https://mycourses.aalto.fi/course/view.php?id=51818))
-   Go to the chat channel #queue.
-   Write a help request (see below), starting with the keyword "Live".
-   When it's your turn in the queue, TA will send you a direct message on
    Zulip, and you will continue the discussion live.
...
help request, a TA should already be able to answer it, or point you to the FAQ.

Try to describe what the problem is, what you have tried, what you
...
I thought that I had an error in ..., so I tried changing ... to ...,
