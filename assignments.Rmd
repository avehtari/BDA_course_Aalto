...
peergrade scores to the correct person. The class code needed for
registration will be posted to all registered students before the course starts.
...
performance to the others. Thus, one should not worry about getting a
...
Chat activity and by answering a time usage questionnaire on the course page in mycourses.
Chat activity and the questionnaire are not included in the maximum assignment
score and thus are not required to receive a full 100\% score.

Other students' Chat activity will not affect one's resulting
grade, i.e. there is no need to try to perform in the Chat or to take any part in it at all.
...
You can get help for the assignments by asking in Chat from other
...
- Written communication on *Chat*: you will chat with a TA using the “direct messages” feature on *Chat*. You can also e.g. share code snippets through Chat direct messages if it helps.
...
We will use the channel **#queue** in Chat to coordinate everything. We announce there when the TA session starts. Then you can write your help request there, describing in sufficient detail exactly what is the problem with which you would need help (see below).
