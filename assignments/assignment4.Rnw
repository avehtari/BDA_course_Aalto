...
\begin{itemize}
    \item[a)] In the prior distribution for $(\alpha,\beta)$, the marginal distributions are $\alpha \sim N(0,2^2)$ and $\beta \sim N(10,10^2)$, and the correlation between them is $\mathrm{corr}(\alpha, \beta)=0.6$. Report the full mean and covariance of the bivariate normal distribution.
\begin{itemize}
