...
\textbf{Grading instructions:} The grading will be done in peergrade. All grading questions and evaluations for assignment 4 can be found \href{https://github.com/avehtari/BDA_course_Aalto/blob/master/assignments/assignment4_rubric.md}{\textbf{here}}
...
assignment_path <-
  "https://github.com/avehtari/BDA_course_Aalto/blob/master/assignments/tests/assignment4.yml"
...
{\bf Bioassay model}. In this assignment, you will use a dose-response relation model that is used in Section 3.7 of the course book and in the chapter reading instructions \href{https://github.com/avehtari/BDA_course_Aalto/blob/master/chapter_notes/BDA_notes_ch3.pdf}{\textbf{here}}. The used likelihood is the same, but instead of uniform priors,
we will use a bivariate normal distribution as the joint prior distribution of the parameters $\alpha$ and $\beta$.


\begin{itemize}
    \item[a)] In the prior distribution for $(\alpha,\beta)$, the marginal distributions are $\alpha \sim N(0,2^2)$ and $\beta \sim N(10,10^2)$, and the correlation between them is $\mathrm{corr}(\alpha, \beta)=0.5$. Report the full mean and covariance of the bivariate normal distribution.
\begin{itemize}
...
the log ratios and scale them such that they sum to one. Below is a test example, the functions can also be tested with \texttt{markmyassignment}.
\begin{itemize}
...
\item \textbf{Note!} {\it BDA3 1st (2013) and 2nd (2014) printing have an error
 ...
 \item[h)] Implement a function for computing the posterior mean using importance sampling, and compute the mean using your 4000 draws. Below is an example how the function would work with the example values for {\tt alpha} and {\tt beta} above.
