...
\textbf{Note!} Both Assignment 8 and 9 build upon this part of the assignment, so it is important to get this assignment correct before you start with Assignment 8 and 9.
...
\begin{itemize}
\item a separate model, in which each machine has its own model
...
Your tasks are the following:
\begin{itemize}
\item [a)] Describe each model (separate, pooled, and hierarchical)
  with mathematical notation (as is done for the separate model
  above). Also describe in words the difference between the three
  models.
\item [b)] Implement the three models in Stan and include the code in
  the report. Use weakly informative priors for all your models.
\item [c)] Using each of the three models with weakly informative
  priors report, comment on and, if applicable, plot histograms for
...
\item [d)] Report the posterior expectation for $\mu_1$ with a 90\%
  credible interval for all three models but using a
  $\operatorname{normal}(0,10)$ prior for the $\mu$ paramater(s) and a
  $\operatorname{Gamma}(1,1)$ prior for the $\sigma$ parameter(s). In
  the hierarchical model, use the $\operatorname{normal}(0,10)$ and
  $\operatorname{Gamma}(1,1)$ as hyper-priors.
\end{itemize}
