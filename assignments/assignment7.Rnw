...
For each of the three models, your tasks are the following:
...
\item [c)] Using the models with weakly informative priors report,
...
\item [d)] Report the posterior expectation for $\mu_1$ with a 90\%
  ...
  for the $\mu$ paramater(s) and a $\operatorname{Gamma}(1,1)$ prior
