...
The provided data \texttt{drowning} in the \texttt{aaltobda} package contains the number of people drown per year in Finland 1980--2019.
%
...
\item The provided Stan code in Listing~\ref{listing1} given on the next page is almost correct for the given problem. However, there are three mistakes. Find these three mistakes and fix them. Report the original mistakes and your fixes clearly in your report. Include the \emph{full} Stan code in your report. You may find some of the mistakes in the code using Stan syntax checker. If you copy the Stan code to a file ending .stan and open it in RStudio (you can also choose from Rstudio menu File->New File->Stan file to create a new Stan file), the editor will show you some syntax errors. More syntax errors might be detected by clicking Check in a bar just on top of the Stan file in RStudio ediot. Note that some of the errors in the presented Stan code can be other than syntax errors.
...
\textbf{Note!} These are \emph{not} results you would expect to turn in in your report. You will need to change the separate model as well.
