...
We have prepared a **[quarto template specific for this
assignment](template7.html)** to help you get started.
...
* Separate model: Is the result for the separate model discussed?
  ...
  -   The result is discussed and the separate model is recognized as **...**.
* Pooled model: Is the result for the pooled model discussed?
  -   The result is not discussed.
  -   There is some discussion, but it is not mentioned that **...**.
  -   The result is discussed and the pooled model is recognized as **...**.
* Hierarchical model: Is the result for the hierarchical model discussed?
  -   The result is not discussed.
  -   There is some discussion, but it is not mentioned that **...**.
  -   The result is discussed and the hierarchical model is recognized as **...**.
::::
...
* Separate model: Is the result for the separate model discussed?
  ...
  -   There is some discussion, but it is not mentioned that **...**.
  -   The result is discussed and the separate model is recognized as **...**.
  -   In addition to the previous option, it is recognized that the separate model only **...**.
* Pooled model: Is the result for the pooled model discussed?
  -   The result is not discussed.
  -   There is some discussion, but it is not mentioned that **...**.
    -   The result is discussed and the pooled model is recognized as **...**.
...
:::{.subtask}
In your report, address the following questions based on the plots you made for both parameterizations:

 - Which of the parameterizations resulted in fewer divergent transitions?
 - Comment for both parameterizations: for which kind of values of $\tau$, do the divergent transitions occur (if there are any)? It also might be that no clear pattern can be seen.
 - Does centered parameterization have problems sampling from some specific region of the parameter space compared to the non-centered parameterization.
:::
::: {.subrubric}
* Has it been discussed which parameterization resulted in fewer divergent transitions?
* Has it been discussed for which values of $\tau$ the divergences occurred?
* Has it been discussed whether/where the centered parameterization has problems sampling?
:::
