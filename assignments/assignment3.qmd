...
This assignment is related to Chapters 2 and 3.  Use [Frank Harrell's
...
* A [recommended template](template3.html) which uses some additional packages, 
which however requires a bit more set-up work to run and
* a [simple template](simple_template3.html) which doesn't use those additional packages and 
is therefore easier to get to run.

::: {.hint}             
**Reading instructions:** 

Chapter 2 and 3 in BDA3, see [**the reading
...
3**](https://avehtari.github.io/BDA_course_Aalto/chapter_notes/BDA_notes_ch3.pdf).
...
to refer to the book instead of deriving the distributions.
    ...
    * Yes
* In part b), were the point estimates and posterior interval provided? (The posterior mean should be
...
* For c)-part, was a formula or a simulation method presented for computing the posterior predictive
...
and use these samples and odds ratio equation to get samples from the
...
have repeated the computations in a)-part which a couple of alternative priors and reported these
...
$\mu_1$ and $\mu_2$, and use these samples to get samples from the
