...
\textbf{Grading instructions:} The grading will be done in peergrade. All grading questions and evaluations for exercise 1 can be found \href{https://avehtari.github.io/BDA_course_Aalto/chapter_notes/assignments/assignment1_rubric.html}{\textbf{here}}
...
exercise_path <-
  "https://github.com/avehtari/BDA_course_Aalto/blob/master/assignments/tests/ex1.yml"
...
\item {\bf (Bayes' theorem)} A group of researchers has designed a new inexpensive and
  ...
  {\bf Hint :} Relatively high false negative (cancer doesn't get detected) or high false positive (un-necessarily administer medication) are typically bad and undesirable in tests.
...
\textbf{Note!} This is a test case, you need to change the number in the matrix to the numbers in the exercise.
