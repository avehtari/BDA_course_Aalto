...
Implement a separate, pooled and hierarchical Gaussian model described in Section 11.6 using Stan. In the pooled model, all the measurements are combined and no distinction is made between the machines. In the separate model, each machine has its model. Similarly, as in the model described in the book, use the same measurement standard deviation $\sigma$ for all the groups in the hierarchical model. In the separate model, however, use separate measurement standard deviation $\sigma_j$ for each group $j$. Use weakly--informative priors. See Stan's recommendations \href{https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations}{\textbf{here}} for suggestions on priors for the different parameters. For the hierachical model, use {\tt scaled\_inv\_chi\_square(4, 1)} priors for the standard deviations.

As an example for the hierachical priors, consider the stan code in Listing~\ref{listing2} for the assignment, where we use {\tt cauchy(4, 1)} priors for the standard deviations.

\begin{lstlisting}[
    float,
    caption=Example priors for the hierarchical model,
    label=listing2,
    basicstyle=\footnotesize\ttfamily,
    numbers=left,
    xleftmargin=2.7em,
...
model {
  ...
  for (i in 1:N)
    for (j in 1:J)
      y[i,j] ~ normal(mu0 + mu[j], sigma);
}
\end{lstlisting}

Under these prior choices, the mean of the posterior for {\tt mu} looks like:

<<echo=FALSE>>=
mu <- c(-6.9663640,  6.5418345, -1.7270129,  8.9439851, -0.7354605, -2.4859669)
@
...
\begin{itemize}
    \item [i)] prior choice discussion for different weakly informative priors,
    \item [ii)] the posterior distribution of the mean of the quality measurements of the sixth machine.
    \item [iii)] the predictive distribution for another quality measurement of the sixth machine.
    \item [iv)] the posterior distribution of the mean of the quality measurements of the seventh machine.
\end{itemize}

\textbf{Note!} The histograms of ii) - iv) will be used as a reference for the pooled model and should be included.
