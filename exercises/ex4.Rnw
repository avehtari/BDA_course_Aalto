...
\textbf{Hint!} The mean and covariance of the bivariate normal distribution are a length-$2$ vector and a $2 \times 2$ matrix. The elements of the covariance matrix can be computed using the relation of correlation and covariance.
  ...
  prior distribution in a) for arbitrary values of $\alpha$ and $\beta$. Below is an example of how the function should be named and how it should work if you want to check your implementation with \texttt{markmyassignment}. 
<<echo=FALSE>>=
...
\item[c)] Implement a function in R for computing the \textbf{logarithm} of the density of the posterior for arbitrary values of $\alpha$ and $\beta$ and data $x$, $y$ and $n$. Below is an example of how the function should be named and how it should work if you want to check your implementation with \texttt{markmyassignment}. 
...
\item[e)] Implement the following two functions: 
\begin{enumerate}
  \item[1)] A function for computing the log importance ratios (log importance weights) for draws from the prior distribution in 1a) when the target distribution is the posterior distribution.
  \item[2)] A function for exponentiating the log importance ratios and normalizing them to sum to one.
  \end{enumerate}
  Below is a test example, the functions can also be tested with \texttt{markmyassignment}.
  
  \textbf{Note!} The values below are \emph{only} a test case, you need to use correct draws for {\tt alpha} and {\tt beta} in the final report. \\
<<echo=FALSE>>=
...
\item \textbf{Note!} {\it BDA3 1st (2013) and 2nd (2014) printing have an error
 ...
 \end{itemize}
\item[h)] Use importance resampling to obtain a posterior sample of size 1000 of $\alpha$ and $\beta$ and plot a scatterplot of the obtained posterior sample.
