...
\begin{enumerate}[a)]
\item Summarize the posterior distribution for the odds ratio, $(p_1/(1-p_1))/(p_0/(1-p_0))$. Compute the point and interval estimates (95\%) and plot the histogram. Use \href{http://www.fharrell.com/2017/10/bayesian-vs-frequentist-statements.html}{Frank Harrell's recommendations} how to state results in Bayesian two group comparison. Below is a test case on how the odd ratio should be computed.
<<echo=TRUE>>=
set.seed(4711)
...
\item What can you say about $\mu_d = \mu_1 - \mu_2$? Summarize your
  results using Bayesian point and interval estimates (95\%) and plot
  the histogram. Use
