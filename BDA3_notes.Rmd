...
unclear to you or things you think might be unclear for others.
...
It is common to write statistical models using the following notation:
$$
\begin{aligned}
...
A collection of distribution statements defines a joint
...
The term $p(y|\theta,M)$ has two different names depending on the situation.
Because of the short notation, confusion is possible.
...
-   In Bayes rule, the term $p(y|\theta,M)$ is called *likelihood
    function*. The posterior distribution describes the probability (or
    ...
    shows explicitly that it is a function of $\theta$. The term has its
    own name (likelihood) to make the difference to the model. The
    likelihood function is an unnormalized probability distribution
...
In that paper, there is one typo: the word *aleatory* instead of
...
- In $p(y|\theta)$
    - $y$ can be a variable or a value
        - we could clarify by using $p(Y|\theta)$ or $p(y|\theta)$
    - $\theta$ can be a variable or a value
    ...
    - $p_\Theta(Y=y|\Theta)$ is a likelihood function (can be discrete
        or continuous)
    - $y$ and $\theta$ can also be a mix of continuous and discrete
    - Due to the sloppiness, sometimes likelihood is used to refer to
...
7, it is sufficient that you know that 1) independence is a stronger
...
related to what information is available instead of the properties of an
...
In the early assignment rounds, we ask you to answer numerical answers with
a fixed number of decimal digits. Later in the course, we will discuss
how to choose the number of significant digits based on the posterior
uncertainty and Monte Carlo standard error.

- For example, 0.01 has two decimal digits and two significant digits
- For example, 0.0099 has four decimal digits and two significant digits
- See more about [significant digits in Wikipedia](https://en.wikipedia.org/wiki/Significant_figures)
- In early assignment rounds, we have intentionally asked for a specified number of decimal digits
...
- 2.3: Use posterior draws to plot a histogram with quantiles, and the same
...
### Posterior draws and sample

"Posterior draws" are sometimes also called "posterior samples". As
sample can also mean data, to improve clarity Stan developers prefer
following: Individual is draw, plural is draws, a set of draws is
sample, and many sets of draws are samples. The number of draws is the
same as the sample size, and later we discuss also effective sample
size (ESS) for a set of posterior draws.

### Posterior, credible, and confidence intervals

- *Confidence interval* is used in frequentist statistics and is not used
...
1745--1770. Laplace used a binomial model and a uniform prior, which produces
...
In case of a Gaussian distribution with known variance $\sigma^2$, the
model is
$$
\begin{aligned}
...
$$
where
$\sigma^2$ describes aleatoric uncertainty. Using a uniform prior, the
posterior is
$$
\begin{aligned}
...
\end{aligned}
$$
where the uncertainty is the sum of epistemic ($\sigma^2/n$) and aleatoric
uncertainty ($\sigma^2$).

### Non-informative and weakly informative priors

Our thinking has advanced since sections 2.8 and 2.9 were written. We're
even more strongly in favor of weakly informative priors, and in favor of
...
also include examples of how seemingly non-informative priors can
...
Thus, the Prior Choice Recommendations Wiki will also see some further updates (we're doing
research and learning more all the time).

### Should we worry about rigged priors?

[Andrew Gelman's blog post answering worries that a data analyst would
choose a too optimistic prior](http://andrewgelman.com/2017/10/04/worry-rigged-priors/).

### Prior knowledge elicitation

Prior elicitation transforms domain knowledge of various kinds into well-defined prior distributions. There are challenges in how to gather domain knowledge and how to transform that to mathematical form. We come back to the topic later in the course. A recent review [*"Prior Knowledge Elicitation: The Past, Present, and Future"* by Mikkola et al. (2023)](https://doi.org/10.1214/23-BA1381) provides more information for those interested to go beoyond the material in this course.
...
[Chapter 3](https://users.aalto.fi/~ave/BDA3.pdf#page=73) is related to Lecture 3 *Multidimensional posterior*.
...
a good example used to illustrate many important concepts, and it is used
...
- 3.1: Visualize joint density and marginal densities of the posterior
    of a normal distribution with unknown mean and variance. [R](https://avehtari.github.io/BDA_R_demos/demos_ch3/demo3_1.html). [Python](https://github.com/avehtari/BDA_py_demos/blob/master/demos_ch3/demo3_1.ipynb).
...
mentions that the conjugate prior for the normal
...
hint is useful to think about the relation between the terms $(n-1)s^2$ and
$n(\bar{y}-\mu)^2$ in 3.2 to equations 3.3 and 3.4.

### Trace of square matrix

Trace of a square matrix, $\trace$,
$\tr A$, $\trace(A)$,
...
try to derive them). As we know how to sample from the posterior of
binomial and normal models, we can use these posterior draws to get
draws from the posterior of the derived quantity.
...
This will be discussed in Lecture 4 and Chapter 10. Meanwhile, e.g., 1000 draws are
sufficient.

### Bioassay

The Bioassay example is an example of a very common statistical inference
...
BDA3). A Swiss company classifies chemicals into different
...
amount of chemical kills 50% of the subjects. The smaller the LD50, the more
lethal the chemical is. The original paper mentions \"1983 Swiss poison
Regulation\" which defines the following categories for chemicals orally
given to rats (mg/ml)\
...
using just 20 rats to define the classification was very little. The book
...
Note that chemical testing is moving away from using rats and other
animals to using, for example, human cells grown in chips, tissue models, and human blood cells. 
The human-cell based approaches are also more accurate at predicting the effect for humans.

$\logit$ transformation can be justified theoretically when a binomial likelihood is used.
...
When asked to compare groups, some students get confused as the
frequentist testing is quite different. Frequentist testing is often
focusing on a) differently named tests for different models and b) null
hypothesis testing. Instead, in Bayesian inference a) the same Bayes rule and
investigation of the posterior distribution is used for all models, b) null hypothesis
...
on continuous model parameters, the probability of 0 difference is 0
(later lecture 12.1 covers null hypothesis testing). Instead of forcing
a dichotomous answer (yes/no) about whether there is a difference, report
the whole posterior that also tells how big that difference might be.
What big means depends on the application, which brings us back to the
fact of the importance of domain expertise. You are not experts on the
application examples used in the assignment, but you can think about how you would
report what you have learned to a domain expert.

[Frank Harrell's recommendations on how to state results in two group
...
BDA3 Section 2.3 discusses posterior summaries and illustrates a bimodal distribution in Figure 2.2.
...
Normal approximation is often used as part of posterior
...
Several researchers have provided partial proofs that the posterior
converges towards a normal distribution. In the mid-20th century Le
Cam was first to provide a strict proof.

### Observed information

When $n\rightarrow\infty$, the posterior distribution approaches
a normal distribution. As the log density of the normal is a
...
only in the case of asymptotic normality. In the case of the normal
distribution, the curvature also describes the width of the
normal. The information matrix is a *precision matrix*, which is
...
Aliasing is a special case of under-identifiability, where the likelihood
repeats in separate points of the parameter space. That is, the likelihood
will get exactly the same values and has the same shape, although possibly
...
$$
has two normals with their own means and variances. With probability
...
other normal component would model faulty measurements.  The model
...
get the equivalent model. The posterior distribution then has two modes
which are mirror images of each other. When $n\rightarrow\infty$, the modes
...
problem. However, aliasing makes the approximate inference more
difficult. 
...
See [BDA3 p. 21](https://users.aalto.fi/~ave/BDA3.pdf#page=31)
for the explanation of how to derive densities for transformed
variables. This explains, for example, why a uniform prior
$p(\log(\sigma^2))\propto 1$ for $\log(\sigma^2)$ corresponds to prior
$p(\sigma^2)=\sigma^{-2}$ for $\sigma^{2}$.

### On derivation

Here's a reminder of how to integrate with respect to $g(\theta)$. For example 
$$
...
[Chapter 5](https://users.aalto.fi/~ave/BDA3.pdf#page=111) is related to Lecture 7 *Hierarchical models and exchangeability*.
...
- 5.5 Example: parallel experiments in eight schools (useful discussion; skip the details of computation)
...
and grid, but there is no need to go deep into computational details as
...
variables/parameters are exchangeable. Exchangeability is a less strict
...
unobserved quantities are exchangeable. ``Ignorance implies
exchangeability.''
...
$J\rightarrow\infty$, but in practice the difference may be small if $J$ is
...
the following three examples, answer: (i) Are observations $y_1$ and $y_2$
...
The following complements the divorce example in the book by discussing the
...
- Example: divorce rate per 1000 population in 8 states of the USA in
  1981
    ...
    - it is reasonable to assume a prior independence given population
      density $p(y_i|\theta)$
- Divorce rates in the first seven are $5.6, 6.6, 7.8, 5.6,
      7.0, 7.2, 5.4$
    - now we have some additional information, but still changing the
      ...
      we were told that divorce rates were not for the first seven but
...
- If observations can be grouped, we may make a hierarchical model, where
  each group has its own subpart, but the group properties are unknown. If
  we assume that group properties are exchangeable, we may use a common
...
pin. This kind of pin can stay flat on its base or slanting so that the
pin head and the edge of the base touch the table. This kind of pin
represents a realistic version of \"unfair\" coin.

- Let's throw the pin $n$ times and mark $x_i=1$ when the pin stands on its
  base. Let's assume that throwing conditions stay the same all the time.
  Most would accept throws as exchangeable.
- Same experiment, but odd numbered throws will be made with a full
  metal pin and even numbered throws with a plastic coated pin. Most
...
- Laboratory experiments $x_1,...,x_n$, are real valued measurements
  ...
  are from the same sample, in the same laboratory with the same
...
- $x_1,...,x_n$ are real valued measurements about the physiological
  ...
  amounts of medicine. Test persons are males and females of different
  ages. If the attributes of the test persons were known, most would
  not accept results as exchangeable. In a group with a certain dose,
...
recommends use of half-Cauchy as a weakly informative
...
values, or half-$t_4$ if there might be possibility of
...
Find all the terms and symbols listed below. When reading the chapter,
write down questions related to things unclear to you or things you
think might be unclear for others.
...
- Gabry, Simpson, Vehtari, Betancourt, and Gelman (2018).
  ...
  Statistical Society Series A*, , 182(2):389-402.<br>
  [Online](https://doi.org/10.1111/rssa.12378).<br>
  [Video of the paper presentation](https://www.youtube.com/watch?v=E8vdXoJId8M).
- Säilynoja, Johnson, Martin, and Vehtari (2025).
  Recommendations for visual predictive checks in Bayesian workflow.<br>
  [Online](https://teemusailynoja.github.io/visual-predictive-checks/)
...
- The paper describing the method used by [priorsense package](https://n-kall.github.io/priorsense/)

1. Kallioinen, Paananen, Bürkner, and Vehtari (2024).
   Detecting and diagnosing prior and likelihood sensitivity with power-scaling.
   [Online](https://doi.org/10.1007/s11222-023-10366-5).

## Chapter 7 Evaluating, comparing, and expanding models {#ch7}

[Chapter 7](https://users.aalto.fi/~ave/BDA3.pdf#page=175) is related to Lecture 8 *Model checking & cross-validation'' and Lecture 9 *Model comparison and selection*.
...
multiplier $-2$ is used. After the book was published, we have concluded
that it causes too much confusion and recommend not multiplying by $-2$.
The above paper is not using $-2$ anymore.

### Extra material

The following article provides an excellent discussion about "How should I
...
ignorable. Chapter 8 explains when data collection can be ignored and
when we also need to model the data collection. We don't have time to go
through Chapter 8 in BDA course at Aalto, but it is highly recommended
that you read it at the end or after the course. The most important
...
accuracy over the range of values where it mostly matters. As the
...
- the largest integer that can be represented is
    $2^{31} - 1 = 2147483647$
...
provides a nice overview of floating-point arithmetic and how the
...
floating-point arithmetic, including examples similar to those shown in the BDA
course lecture.
- [Stan User Guide Chapter 15](https://mc-stan.org/docs/2_26/stan-users-guide/floating-point-arithmetic.html)
discusses floating point arithmetic in the context of Stan.

### Terms draw, draws, and sample

A group of draws is a sample. A sample can consist of one draw, and
thus some people use the word sample for both a single item and for the
...
error. BDA3 Section 10.5 discusses the basics of this. Additional
information:

- When we know the needed accuracy for reporting posterior summaries,
  we can estimate how big a sample size is needed to get a small enough
  MCSE for the quantity of interest.
- MCSE is based on the central limit theorem, which assumes that the
...
- [`posterior`](https://mc-stan.org/posterior/) package provides
  ...
  probabilities is obtained using `mcse_mean()` for the indicator function
...
- [Wikipedia Central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem) article has a good summary of the topic and variants with links to further information
...
- $\hat{k}$ estimate has its own variation given a finite sample size,
  and if in doubt, get more draws to get a more accurate $\hat{k}$
  estimate.
- Pareto-$\hat{k}$ diagnostic is pre-asymptotic based on finite sample
  size, and indicates the stability of the empirical mean estimate given
...
- Too many digits make reading the results slower and give a false
...
- Don't ever show digits which are just random noise. You can use Monte
  ...
  to stay the same if the sampling were continued.
- The advice considers the number of significant digits, which is
...
- Example: The probability that temperature increase is positive
    ...
    - For probabilities close to 0 or 1, consider also when the model
      assumption justifies certain accuracy
- When reporting many numbers in a table, for aesthetic reasons, it may
  be sometimes better for some numbers to show one extra or one too
  few digits compared to the ideal.
- Often it's better to plot the whole posterior density in addition to
  any summaries, as summaries always lose some information content.
- For your reports: Don't be lazy and settle for the default number of
  ...
  are sensible. If the Quiz asks for a certain number of digits, this overrules
  other advice.
...
distribution is often done using [Ziggurat
...
The popularity of importance sampling is increasing. It is used, for
example, as part of other methods such as particle filters and pseudo
...
BDA3 1st (2013) and 2nd (2014) printings have an error for
...
The effective sample size estimate mentioned in the book is a generic
approximation, and a more accurate effective sample size estimate would
also take into account the functional. For example, importance sampling
effective sample size can be different when estimating
$\E[\theta]$ or $\E[\theta]^2$. If you
are interested in more details, for example, [the Pareto smoothed importance
...
the experiment in addition to describing the approach for estimating
...
[Chapter 11](https://users.aalto.fi/~ave/BDA3.pdf#page=285) is related to Lecture 5 *Markov chain Monte Carlo*.

### Outline

- Markov chain simulation: before Section 11.1, pages 275-276
- 11.1 Gibbs sampler (an example of a simple MCMC method)
- 11.2 Metropolis and Metropolis-Hastings (an example of a simple MCMC
    method)
...
have a nice review of the fundamental terms. See, especially, the slides
for lecture 4. To prove that Metropolis algorithm works, it is
sufficient to show that the chain is irreducible, aperiodic, and not
transient.
...
There is a lot of freedom in the selection of the proposal distribution in
Metropolis algorithm. There are some restrictions, but we don't go to
the mathematical details in this course.

Don't confuse rejection in rejection sampling and in Metropolis
algorithm. In rejection sampling, the rejected samples are thrown
...
When rejecting a proposal, the previous sample is repeated in the chain; it has to be included, and 
it is a valid sample from the distribution. 
For basic Metropolis, it can be shown that the optimal
rejection rate is 55%--77%, so that even in the optimal case quite many
...
the time than more often to jump really close. Methods for estimating the
effective sample size are useful for measuring how effective a given
chain is.

### Transition distribution vs. proposal distribution

The transition distribution is a property of a Markov chain. In the Metropolis
algorithm, the transition distribution is a mixture of a proposal
distribution and a point mass at the current point. The book also uses the
term jumping distribution to refer to the proposal distribution.

### Convergence

Theoretical convergence in infinite time is different than practical
convergence in a finite time. There is no exact moment when the chain has
...
even if would be able to start from an independent sample from the
posterior so that the chain starts from the convergence, the mixing can be
so slow that we may require a very large number of samples before the
samples are representative of the target distribution.

If the starting point is selected at or near the mode, less time is needed
...
unless the starting point was somehow sampled directly from the target
distribution.
...
were updated in BDA3, and a slightly updated version of this is described
...
provides a summary of available convergence diagnostics in Stan and how to
interpret them.
...
this misconception). All the posterior draws in an autocorrelated Markov
...
a property of individual draws, but a joint property of all draws in a
...
simulation is not adaptively selected) is
...
Stan uses a variant of dynamic Hamiltonian Monte Carlo (using an adaptive
...
The No-U-Turn Sampler gave the name NUTS, which you can often see
...
- [Brief Guide to Stan's Warnings](https://mc-stan.org/misc/warnings.html)
  provides a summary of available convergence diagnostics in Stan,d how
...
- I recommend starting with these
    - Bob Carpenter, Andrew Gelman, Matt Hoffman, Daniel Lee, Ben
      ...
      Guo, Peter Li, and Allen Riddell (2017). [Stan: A Probabilistic Programming Language](https://www.jstatsoft.org/article/view/v076i01). Journal of Statistical Software, 76(1), 1–32.
...
- More complete information (no need to read from the beginning to the end, but use when needed)
...
transpiled to C++ code, and then that C++ code is compiled to machine
...
Using a transpiler and compiler allows to develop Stan language to be good
...
[Chapter 4](ch4) presented the normal distribution approximation at the
...
- Finding posterior modes
    ...
    - Stan uses limited-memory Broyden-Fletcher-Goldfarb-Shannon
      (L-BFGS) which is a quasi-Newton method that needs only the
      first derivatives (provided by Stan autodiff). L-BFGS is known
      for good performance for a wide variety of functions.
...
- Conditional and marginal posterior approximations
    - Even in the time of efficient probabilistic programming, the
      ...
      important part of popular INLA software and are also coming to
      Stan to speed up latent Gaussian variable models.
...
- Expectation propagation
    - Practical efficient computation for expectation propagation (EP)
      is applicable for a more limited set of models than post-BDA3
...
- Other approximations
    - Just brief mentions of INLA (in 13.5),
...
- Regression for causal inference: incumbency and voting
    - Modeling example with a bit of discussion on causal inference
...
- Including numerical prior information
    - useful conceptually, but easier computation with probabilistic
    ...
    - the discussion of fixed-, random- and mixed-effects models is
      incomplete
        ...
        - often fixed effects correspond to population level
          ...
          individual level coefficients, and a mixed model has both

          $$
          \begin{array}{ll}
          \hline
          \text{Model} & \text{Interpretation} \\
          \hline
          y \sim 1 + x
            & \text{fixed / population effect; pooled model} \\[4pt]
          y \sim 1 + (0 + x \mid g)
            & \text{random / group effects} \\[4pt]
          y \sim 1 + x + (1 + x \mid g)
            & \text{mixed effects; hierarchical model} \\
          \hline
          \end{array}
          $$
...
- Computation: batching and transformation
    ...
    - transformations for HMC are useful if you write your own models,
...
- Analysis of variance and the batching of coefficients
    ...
    - rstanarm and brms packages make it easy to run ANOVA
...
The model in the Bioassay example in [Chapter 3](ch3) is also a generalized linear model.
The chapter reviews the basics and discusses some computational issues, but
probabilistic programming frameworks make computation for generalized
linear models easy (especially with rstanarm and brms). See [ROS](https://avehtari.github.io/ROS-Examples/) for discussion of generalized linear models from the modeling perspective.
...
- Parts of generalized linear model (GLM):
    ...
    - Outcome distribution model with location parameter $\mu$
        - the distribution can also depend on a dispersion parameter $\phi$
        ...
        - after MCMC made computation easy, GLM can refer to models
          where the outcome distribution is not part of the exponential family
          and the dispersion parameter may have its own latent linear
          predictor
- Standard generalized linear model likelihoods
    ...
    - continuous data: normal, gamma, Weibull mentioned, but common
      ...
      extreme value distributions like the generalized Pareto distribution
    ...
    - Poisson for count data with no upper limit
        - Poisson is a useful approximation of Binomial when the
          observed counts are much smaller than the upper limit
- Working with generalized linear models
    - A bit of this and that information on how to think about GLMs (see
      [ROS](https://avehtari.github.io/ROS-Examples/) for more)
    - normal approximation to the likelihood is good for thinking about how
...
- Weakly informative priors for logistic regression
    ...
    - the computation part is outdated, as probabilistic programming
...
- Aspects of robustness
    - overdispersed models are often connected to robustness of
      ...
      overdispersed without any observation being an outlier
    - outlier is sensible only in the context of the model, being
      something not well modeled or something requiring an extra model
      component
    - switching to a generic overdispersed model can help to recognize
      problem in the non-robust model (sensitivity analysis), but it
      can also throw away useful information in the "outliers" and it
      would be useful to think about what is the generative mechanism for
      observations that are not like others
...
- Robust inference for the eight schools
    - eight schools example is too small to see much difference
