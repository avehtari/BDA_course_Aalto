...
* Choose `CS-E5710 - Bayesian Data Analysis (2026).`
...
* In the RStudio Files pane (bottom right), you can create folders for your work and upload files from your computer to the server
    ...
    * You may get an error when uploading a large zip file, but uploading smaller zip files works. If you can't upload the demo zip file, contact the course staff via Zulip.
...
* After uploading files, use the Files pane to open them (e.g., an R Markdown or Quarto notebook)
* Knitting of R, Rmd, and qmd files works as well (tested 24th August)
* CmdStanR used later in the course has been tested to work on 24th August.
    * To use CmdStanR:<br>
      `library(cmdstanr)`<br>
      * If you do not see the following output, please contact us on Zulip:
      `This is cmdstanr version 0.9.0`<br>
      `- CmdStanR documentation and vignettes: mc-stan.org/cmdstanr`<br>
      `- CmdStan path: /coursedata/cmdstan`<br>
      `- CmdStan version: 2.39.0`<br>
* There is limited memory available (3Gib), and bigger models and datasets can run out of memory with a cryptic error message, but the demos and assignment models should run (if not, then contact the course staff via Zulip).
* See also [Aalto JupyterHub FAQ and bugs](https://scicomp.aalto.fi/aalto/jupyterhub/#faq-and-bugs)

###  Option 2: Use Aalto Linux via the remote desktop solution provided by Aalto-IT.
...
* Download the VMware Horizon application or use the web portal
...
* Go to `jupyter.cs.aalto.fi` in your favorite web browser. ![](https://zenodo.org/record/4010716/files/1.png)

* Log in with your Aalto username and password.

* Select the `CS-E5710 - Bayesian Data Analysis (2026) -r-ubuntu:6.2.3-bayesda2026` server.
...
* Now you should have an RStudio-like interface in your web browser. Click on `File -> Open File...`
...
* Select a demo to run. Here, we open the folder `demos_ch2`, then select `demo2_1.R` file and click `Open`.
...
Instead of trying to download each file separately via the GitHub interface, it is recommended to use one of these options:
 ...
 - If you don't want to learn to use git, download the repository as a ZIP file. Click the green button "Code" on the main page of the repository and choose "Download ZIP" ([direct link](https://github.com/avehtari/BDA_course_Aalto/archive/master.zip)). Remember to download again during the course to get the latest updates.
...
Aalto JupyterHub. To install the package on your own computer, just run the following:

1. `install.packages("aaltobda", repos = c("https://avehtari.r-universe.dev", getOption("repos")))`

If during the course there is an announcement that `aaltobda` has been
updated (e.g. some error has been fixed), you can get the latest
version by repeating the second step above.

### Error related to LC_CTYPE while installing the `aaltobda` R package.
...
**Solution:**
See the following Stack Overflow solution. ([link](https://stackoverflow.com/a/3909546))
...
```
You can also install packages from the RStudio menu `Tools->Install Packages`.
...
Instead of RStan, you can also use the new `CmdStanR` which may be easier to install.

### Workarounds for current Rstan Windows issues

* [A list of the current RStan Windows problems and known temporary workarounds](https://discourse.mc-stan.org/t/workarounds-for-current-rstan-windows-issues/17389)

## How to render a single qmd-file to HTML or PDF

Clicking Render in RStudio defaults to rendering all qmd files in the directory ("project") and to render both HTML and PDF. To save time in rendering, you can install the `quarto` R package
```
...
`quarto` R package is available pre-installed in JupyterHub (you may need to restart your server to get the new image).

If rendering to PDF doesn't work, you can print the HTML to a PDF file, but do make sure to turn off "More setings -> Print headers and footers" to avoid accidentally printing your identity.
...
* A clear characteristic that distinguishes tidyverse from base R is the [pipe operator](https://style.tidyverse.org/pipes.html) `%>%`
...
Unfortunately, the installation of `pystan` will fail on an M1 Mac, as there is no binary wheel available for the `httpstan` dependency. A recommended alternative here is the [CmdStanPy package](https://mc-stan.org/cmdstanpy/).

M1 Mac users who are intent on using `pystan` will need to complete the following steps to build `httpstan` from source and then install `pystan`:
...
## I missed some deadlines or wasn't able to do some part of the course
...
- I missed the deadline to register for the course in Sisu. Can I join the course?
    - Yes, just register in MyCourses and contact student services so they add you in Sisu, too.
- I missed the deadline for the assignment. Can you accept my late submission?
    ...
    <!-- - If you miss the FeedbackFruits deadline the first time it's used due to technical problems, but you send the PDF to one of the TAs a few minutes after the deadline, it can be accepted once. As the recommended submission time is before 4 pm on Friday, you have in general 56 hours extra hours for submission, and several few-minute late submissions are not likely to be accepted just due to technical problems. -->
- I was not able to do one of the assignments because [some personal problem]. Can I do some extra work?
    - Things happen, and you don't need to tell the course staff your personal reasons (especially you shouldn't tell any health issue details). Everyone gets a second chance in period III. In period III, there is just one submission deadline, but otherwise the procedure is the same. If you submitted the project work in autumn, you don't need to re-submit it if you re-submit assignments.
- I missed the deadline to register for a project group. Can I still register?
...
- I was not able to a) do the project or b) give a presentation because [some personal problem]. Can I a) submit it later, b) present later?
    - Things happen, and you don't need to tell the course staff your personal reasons (especially you shouldn't tell any health issue details). Everyone gets a second chance in period III. In period III, there is a second project submission deadline and presentation slots. If you are happy with your assignment score, you don't need to re-submit assignments if you submit the project work in period III.

## Recommended courses after Bayesian Data Analysis

Here are some great Aalto courses that use Bayesian inference
