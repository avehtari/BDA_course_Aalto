...
\begin{frame}
  ...
  \item Frequentist statistics accepts only aleatory probabilities
    ...
    \item Uncertainty of estimates are based on all possible data
      ...
      \end{itemize}
    \end{itemize}
  \item<3-> Estimates are derived to fullfil frequency properties
    \begin{itemize}
    \item Maximum likelihood fullfills just asymptotic frequency
      properties
...
\begin{frame}
  ...
  \begin{itemize}
  \item Estimates are derived to fullfil frequency properties
    \begin{itemize}
    \item Maximum likelihood fullfills just asymptotic frequency
      properties
    \item Common desiderata are 1) unbiasedness, 2) minimum
...
\begin{frame}
  ...
  \begin{itemize}
  \item Instead of hypothesis testing, report full posterior and
    combine full posterior with utility/cost function
    \begin{itemize}
    \item for continuous posterior there is zero probability that
      e.g. treatment effect is exactly zero
    \end{itemize}
  \item<2-> Equivalence testing
    \begin{itemize}
    \item e.g. what is the probability that the treatment is closer
      than $\epsilon$ to zero, where $\epsilon$ is chosen based on
      what is practically useful treatment effect size
    \end{itemize}
  \item<3-> Bayes factor
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{tabular}{ll}
   $t$-test &  {\tt stan\_glm(y \~\ 1)}\\
   paired $t$-test &{\tt stan\_glm((y\_1 - y\_2) \~\  1)}\\
   pearson correlation &{\tt stan\_glm(y \~\  1 + x)}\\
   two-sample $t$-test &{\tt stan\_glm(y \~\  1 + gid)}\\
    $\ldots$ &
  \end{tabular}
