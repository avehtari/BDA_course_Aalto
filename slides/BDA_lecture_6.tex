...
\begin{frame}{Other packages}
  ...
  \item R
    \begin{itemize}
    \item \texttt{posterior} --- posterior handling and diagnostics\\ (Lecture 5)
    \item \texttt{shinystan} --- interactive diagnostics
    \item \texttt{bayesplot} --- visualization and model checking\\ (Lectures 5 and 8)
    \item \texttt{tidybayes} and \texttt{ggdist} -- more posterior and prediction visualization
    ...
    \item \texttt{priorsense} --- prior and likelihood sensitivity
      diagnostics\\ (can be useful in project)
    \end{itemize}
    \vspace{\baselineskip}
  \item Python
    \begin{itemize}
