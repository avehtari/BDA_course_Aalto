...
\end{minted}
  ...
  \only<2>{\rinline/pp_check(fit, ndraws=20)/\\
    ...
    \begin{minipage}[t][][t]{0.33\linewidth}
      \vspace{-3cm}
      {\color{red} Continuous kernel density estimate
        for binary target doesn't make sense!}
    \end{minipage}
  }
  \only<3>{\rinline/pp_check(fit, type="bars")/\\
    \includegraphics[height=3.5cm]{helicopter_hier_stable_ppc_bars_2024.pdf}
    \begin{minipage}[t][][t]{0.33\linewidth}
      \vspace{-3cm} {\color{red} Bar plot showing marginal
        probabilities for binary data is almost always useless!}
    \end{minipage}
  }
...
\begin{frame}{Further reading and examples}
  ...
  \item Säilynoja, Johnson, Martin, and Vehtari (2025).
    Recommendations for visual predictive checks in Bayesian workflow.
    \url{https://teemusailynoja.github.io/visual-predictive-checks/}
