...
\begin{frame}{Binomial: computation}
  ...
  \item Python
    ...
    \end{itemize}
    \begin{itemize}
        \item {\tt from preliz import Beta}
        \item Similar to  scipy.stats
        \item Nicer features to explore distributions 
        \item \url{https://preliz.readthedocs.io}
    \end{itemize}
...
\begin{frame}{Informative prior for left handedness}
  ...
  \only<4>{\vspace{0.5\baselineskip}\includegraphics[width=6cm]{figs/beta_8_60_prior.pdf} \small Demo:\url{https://handedness.streamlit.app}}
\end{frame}
...
\begin{frame}{Binomial model with $\theta=f(x)$}
  ...
  \includegraphics[width=10.5cm]{figs/helicopter_results_all_years.pdf}
...
\begin{frame}{Reasons to use Normal distribution}
  ...
  \begin{itemize}
    ...
    \item Tradition
    \item Often good approximation (justification based on central limit theorem)
  \end{itemize}
...
\begin{frame}{Central limit theorem*}
  ...
  \item<2-> Problems
    \begin{itemize}
    \item<2-> does not hold for distributions with infinite variance,
      e.g., Cauchy
    \item<3-> may require large $n$,\\ e.g.
      Binomial, when $\theta$ close to $0$ or $1$
    \item<4-> does not hold if one the variables has much larger scale
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Thinking priors}
  ...
  \item Make a guess of some quantities and then find out useful
    ...
    \item proportion of students who are taller than 1.9 m
