...
\begin{frame}{Normal / Gaussian}
  ...
  \only<1-6>{observation $y$, and parameters $\mu$ and $\sigma$}
  \only<7>{
    {observation $y$, and parameters $\mu=1$ and $\sigma=1$}
    }
  \only<8>{
    {observation $y$, and parameters $\mu=1$ and $\sigma=1/2$}
    }\only<1>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&=\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{1}{2\sigma^2}({y}-\mu)^2\right)
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal6}
  }\only<2>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&=\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{1}{2\sigma^2}{\color{red}({y}-\mu)^2}\right)
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal2}
  }\only<3>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&=\frac{1}{\sqrt{2\pi}\sigma}\exp\left({\color{red}-}\frac{1}{2\sigma^2}{\color{red}({y}-\mu)^2}\right)
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal3}
  }\only<4>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&=\frac{1}{\sqrt{2\pi}\sigma}\exp\left({\color{red}-\frac{1}{2\sigma^2}({y}-\mu)^2}\right)
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal4}
  }\only<5>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&=\frac{1}{\sqrt{2\pi}\sigma}{\color{red}\exp\left(-\frac{1}{2\sigma^2}({y}-\mu)^2\right)}
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal5}
  }\only<6>{
  \begin{align*}
    p({y} \mid \mu,\sigma)&={\color{red}\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{1}{2\sigma^2}({y}-\mu)^2\right)}
  \end{align*}
    \centering
  \includegraphics[height=4cm,clip]{lec3_normal6}
  }\only<7-8>{
    ...
    \centering
    \includegraphics[height=4cm,clip]{lec3_normal7}
    }\only<8>{
    \centering
    \includegraphics[height=4cm,clip]{lec3_normal8}
    }
...
\vspace{2\baselineskip}
  ...
  \centering
\only<1>{
  \begin{minipage}[t][][t]{1.0\linewidth}
    \hspace{-5mm}\includegraphics[height=4cm,clip]{helicopter_hier_time_2025.pdf}
  \end{minipage}
}
\only<2>{\includegraphics[height=4cm,clip]{helicopter_hier_time_residual_2025.pdf}}
\only<3>{\includegraphics[height=4cm,clip]{helicopter_hier_time_residual_hist_2025.pdf}}
\only<4>{\includegraphics[height=4cm,clip]{helicopter_hier_time_residual_dots_2025.pdf}}
\only<5->{\includegraphics[height=4cm,clip]{helicopter_hier_time_residual_dots_normal_2025.pdf}}

\uncover<6->{
  \begin{itemize}
  \item Why close to normally distributed?
    \begin{itemize}
    \item<7-> Many small sources of variations summed together tend to
      be close to normally distributed
    \item<8-> Central Limit Theorem (CLT)
    \end{itemize}
  \end{itemize}
  }
...
\begin{frame}{Monte Carlo and posterior draws}
  ...
  \only<1-2>{Density $p(\theta \mid \mu,\sigma)=\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{1}{2\sigma^2}(\theta-\mu)^2\right)\quad$ {\color{gray}(\texttt{dnorm()})}\\
    \includegraphics[width=10cm]{norm1d_1.pdf}
  \uncover<2>{$\E(\theta)=\int \theta p(\theta \mid \mu,\sigma) d\theta = \mu$}}
  \only<3>{Density $p(\theta \mid \mu,\sigma)=\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{1}{2\sigma^2}(\theta-\mu)^2\right)$\\
    \includegraphics[width=10cm]{norm1d_1b.pdf}
    {$p(\theta \leq 0)=\int_{-\infty}^{0} p(\theta \mid \mu,\sigma) d\theta$},\,\,\\
...
\begin{frame}{Comparison of means of two normals}
  ...
  \item The difference of two $t$ distributed variables with different
    ...
    \begin{itemize}
    \item but easy to sample from the two distributions, and obtain
      draws of the differences
      \begin{align*}
...
\begin{frame}{Multivariate normal}
  ...
  \item Observation model
    ...
    \end{align*}
  \item BDA3 p. 72--
  \item Recommended LKJ-prior mentioned in Appendix A, see more
    in Stan manual
...
\begin{frame}{Bioassay}
  ...
  \only<3->{
    \vspace{-1.5\baselineskip}
        \begin{align*}
      \mbox{LD50:}\;\;\;
          \E[\theta]=\logit^{-1}(\alpha+\beta x) = 0.5
...
\begin{frame}{Example: GLM for counts}
  ...
  \vspace{-1\baselineskip}
    ...
    \begin{align*}
      ...
      \uncover<2->{\\ {\Poisson(y_i \mid \mu_i )} & = \frac{1}{y_i!}\mu_i^{y_i}e^{-\mu_i}}
      \uncover<3->{\\ ~ \\ \text{Alternatively}: & \\
      y_i \mid \color{blue}\mu_i,\color{black}\phi & \sim \Negbin(y_i \mid \color{blue}\mu_i,\color{black}\phi)}
    \end{align*}
    \uncover<4->{ \tiny 
    \begin{equation*}
