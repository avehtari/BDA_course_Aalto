...
\begin{frame}{Outline}
  ...
  \begin{itemize}
  \item Variable selection with projpred
  \item Bayesian software for Python users
  \end{itemize}
...
\begin{frame}{Variable selection}
  ...
  \begin{itemize}
  \item The process of identifying the most relevant variables in a model from a larger set of predictors. 
  \item We assume variables contribute unevenly to the outcome.
  \begin{itemize}
      \item We may want to identify the most "important" ones.
      \item Sometimes we also want to rank them.
  \end{itemize}
  \end{itemize}
...
\begin{frame}{Motivation for variable selection}

  \begin{itemize}
  \item<+->  We can always include all available variables in a model
  \item<+->  Theory says this is a good idea, in particular if we use predictively consistent priors
  \item<+->  However, sometimes we need to reduce the number of variables
    \begin{itemize}
...
\begin{frame}{The problem with variable selection}

  \begin{itemize}
  \item The number of potential models is $2^p$, where $p$ is the number of variables
  \item Evaluating all models can be computationally infeasible even for moderate $p$
  \item The proccess is prone to overfitting 
  \end{itemize}
  
\end{frame}


\begin{frame}{How to overcome the problem?}

  \begin{itemize}
  \item We recommend to use a technique called projection predictive inference
  \item It can be easily done with \texttt{brms} + \texttt{projpred}
  \end{itemize}
...
\begin{frame}{Variable selection with projpred}
  ...
  \begin{itemize}
  \item The main advantage is that it reduces overfitting
  \item Other advantages are:
    \begin{itemize}
    \item Automatic model building and fitting process.
    \item Reduced number of models we need to fit.
    \item Reduced time it takes to fit each model.
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Main concepts}
  ...
  \begin{itemize}
  \item<1-> Reference model:
    \begin{itemize}
      \item<2-> A model that includes all available variables and describes the data well.
    \end{itemize}
  \item<1-> Search strategy: 
    \begin{itemize}
      \item<3->  A method for searching through the model space.
    \end{itemize}
  \item<1-> Projection:
      \begin{itemize}
      \item<4->  A way to estimate the posterior distribution of a model given a reference model.
    \end{itemize}
...
\begin{frame}{Useing a reference model is not a novel idea}

  \begin{itemize}
  \item<+-> Lindley (1968): \textit{The choice of variables in multiple regression}
    \begin{itemize}
      % \vspace{-1\baselineskip}
    \item Bayesian and decision theoretical justification, but simplified model and computation
    \end{itemize}
...
\begin{frame}{Projection}
  \begin{itemize}
  \item<1-> Replace full posterior {\color{set11} $p(\theta \mid D)$} with
    some constrained {\color{set12} $q(\theta)$} so that the {\color{set13} \emph{predictive
      distribution}} changes as little as possible
  \item<2-> Example constraints
    \begin{itemize}
      \item {\color{set12} $q(\theta)$} can have only point mass at some $\theta_0$ $\Rightarrow$ ``Optimal point estimates''
      \item<3-> Some covariates must have exactly zero regression coefficient $\Rightarrow$ ``Which covariates can be discarded''
      \item<4-> Much simpler model $\Rightarrow$ ``Easier explanation''
    \end{itemize}
  \end{itemize}
\end{frame}
...
\frame{
  ...
  {\Large\color{navyblue} Searching for submodels}

\begin{itemize}
	\item We may not be able to evaluate the $2^p$ combinations \pause
  ...
  \item Use cross-validation to select the appropriate model size
        \begin{itemize}
        \item In some cases like, $p>>n$, we need to cross-validate over the search paths
        \end{itemize}
\end{itemize} 
}
...
\begin{frame}{Multilevel regression and GAMMs}
...
\end{minted}
\item If $p$ is very big use subsampling loo\\
\begin{minted}[fontsize=\footnotesize]{r}
# nloo should be a positive integer smaller than the number of observations
...
\begin{frame}{Bayesian Python packages}

  \begin{itemize}
  \item Probabilistic programming languages
    \begin{itemize}
    \item Stan (via \href{https://mc-stan.org/cmdstanpy/}{CmdStanPy})
    \item \href{https://www.pymc.io/}{PyMC}
    \item NumPyro
    \item ...
    \end{itemize}
  \item Workflow packages
    \begin{itemize}
      \item \href{https://python.arviz.org/en/stable/}{ArviZ}, MCMC diagnostics, model checking, model comparison, plotting, prior-sensitivity...
      \item \href{https://bambinos.github.io/bambi}{Bambi}, BAyesian Model-Building Interface
      \item \href{https://kulprit.readthedocs.io}{Kulprit}, projective inference
    \end{itemize}
  \end{itemize}

\href{https://github.com/avehtari/BDA_course_Aalto/blob/master/slides/BDA_lecture_11c.ipynb}{\color{blue}{Download}} Jupyter Notebook with examples.
