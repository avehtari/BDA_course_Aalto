...
\begin{frame}{How many simulation draws are needed?}
  ...
  \item If
    ...
    \begin{itemize}
    \item<2-> this variance is independent on dimensionality of $\theta$
    ...
    %   \pause
    \item<3-> See BDA3 Ch 4 for counter-examples for asymptotic normality
      \pause
    \item<4-> $\sigma_\theta/\sqrt{S}$ is called Monte Carlo standard
      error (MCSE)
      \item<5-> In practice, $\sigma_{\theta}$ will be estimated by $ \sqrt{1/(S-1)\sum_{s = 1}^S (\theta^{(s)} - \E(\theta))^2}$
    \end{itemize}
\end{itemize}
...
\uncover<13>{See also \url{https://users.aalto.fi/~ave/casestudies/Digits/digits.html}}
...
\begin{frame}[fragile]{Pareto-$\hat{k}$ in \texttt{posterior} package}{Thanks to Noa Kallioinen}

  {\color{gray}
\begin{verbatim}
> drt |> summarise_draws(mean, mcse_mean)
\end{verbatim}
    }
\begin{verbatim}
...
\begin{verbatim}
 variable  mean mcse_mean    khat
 xn       0.010     0.021  -0.072
 xt3      0.025     0.033   0.33 
 xt2_5    0.031     0.039   0.41 
 xt2      0.046     0.054   0.52 
 xt1_5    0.092     0.13    0.69 
 xt1      0.33      1.5     1.0  
\end{verbatim}
...
\begin{frame}{When to use Pareto-$\hat{k}$ diagnostic}
  ...
  \item<+-> To check posterior of any quantity of interest
    ...
    \item after this, check my video + case study on how many
      iterations to run digits to report
    \end{itemize}
