...
\begin{frame}[fragile]
  ...
  \item Normal approximation can be computed numerically
    ...
    \item autodiff or finite-difference for gradients and Hessian (second derivatives)
    \item covariance matrix is inverse of negative Hessian matrix
    \end{itemize}
  \end{itemize}
\end{frame}
...
theta0 <- c(0,0)
optimres <- optim(theta0, bioassayfun, gr=NULL, df1, hessian=T)
...
\begin{frame}{Normal approximation}
  ...
  \item Optimization and computation of Hessian requires usually much
    fewer density evaluations than MCMC
...
\begin{frame}{Normal approximation and parameter transformations}
  ...
  \item<+-> Normal approximation is not good for parameters with
    ...
    \item<+-> Stan code can include constraints\\
      \rinline/real<lower=0,upper=1> theta;/
...
\begin{frame}{Other distributional approximations}
    ...
    \item<+-> Instead of mode and Hessian at mode, e.g.
      ...
      \item variational inference (Ch 13)
        ...
        \item Stan has Pathfinder algorithm (CmdStanR, brms)
...
\begin{frame}{Pathfinder: Parallel quasi-Newton variational inference.}
  ...
  \vspace{-\baselineskip}
    \small
 Birthdays case study uses Pathfinder to
    speed up workflow\\
 \url{https://users.aalto.fi/~ave/casestudies/Birthdays/birthdays.html}\\
\vspace{0.25\baselineskip}
\includegraphics[width=9cm]{fig-births-fit8rhs-vs-data-1.pdf}
...
\begin{frame}[fragile]{brms supports Laplace / Pathfinder / ADVI}

  These might be useful for initializing MCMC or big data. The ADVI
  implementation is not very good.
  
\begin{minted}[fontsize=\footnotesize,escapeinside=\%\%]{r}
fit1 <- brm(..., %\highlight{algorithm = "laplace"}%)

fit1 <- brm(..., %\highlight{algorithm = "pathfinder"}%)

fit1 <- brm(..., %\highlight{algorithm = "meanfield"}%)
...
\begin{frame}
  ...
  \item<3-> A lot of machine learning is not pure frequentist or
    Bayesian, but there is often a probabilistic flavor
  \end{itemize}
