...
\begin{frame}{Extra material for HMC / NUTS}
  ...
  \item A review of why HMC works:\\
    Radford Neal (2012). MCMC using Hamiltonian
    dynamics. \url{https://arxiv.org/abs/1206.1901}
  \item The No-U-Turn Sampler:\\
    Matthew D. Hoffman and Andrew Gelman (2014). The No-U-Turn
  ...
  \item Multinomial variant of NUTS:\\
    Michael Betancourt (2018).  A Conceptual Introduction to
...
\begin{frame}{Hamiltonian Monte Carlo}
  ...
  \item Previously
    \begin{itemize}
    \item Factorizing: sample from 1) $p(\sigma^2)$, 2) $p(\mu \mid \sigma^2)$
    \item Gibbs: sample from 1) $p(\sigma^2 \mid \mu)$, 2) $p(\mu \mid \sigma^2)$
    \end{itemize}
  \item<2-> HMC
    \begin{itemize}
    \item Augment with $\phi$ (the same dimensionality as $\theta$)
    \item Sample from 1) $p(\phi)$, 2) $p(\theta,\phi)$
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Hamiltonian Monte Carlo}
  ...
  \item[1)] Sample from $p(\phi)$
    ...
    \item define $p(\phi) = \normal(0,1)$
    \end{itemize}
  \item[2)] Sample from $p(\theta,\phi)$
    \begin{itemize}
    \item Hamiltonian dynamic simulation
    \end{itemize}
...
\begin{frame}{Hamiltonian dynamic simulation}
  ...
  \item Statistical mechanics and canonical distribution
    ...
    \end{align*}
    where $H$ is Hamiltonian energy function, $U$ is potential energy
    function, $K$ is kinetic energy function, and $\phi$ can now be
    considered to be a momentum variable
  \item<2-> The potential energy is the neg log density\\
    $U(\theta)=-\log(p(\theta))+C$
  \end{itemize}
...
\begin{frame}{No-U-Turn sampler}
    ...
    \item Adaptively selects number of steps to improve robustness and
      ...
      \item simulate until a U-turn is detected
      \item the number of simulation steps doubled if no U-turn yet
      \end{itemize}
    \item<2-> To keep reversibility of Markov chain
      \begin{itemize}
    ...
    \item<3-> For further efficiency
      ...
      \end{itemize}
    \item<4-> Demo \url{https://chi-feng.github.io/mcmc-demo/}
    \end{itemize}
