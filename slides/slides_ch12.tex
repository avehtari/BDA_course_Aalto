...
\pdfinfo{
  /Title      (Bayesian data analysis 12)
  /Author     (Aki Vehtari) %
...
\begin{frame}
  ...
  \item Stan development team (2018). Modeling Language User's Guide
    and Reference Manual
    \url{https://github.com/stan-dev/stan/releases/download/v2.17.0/stan-reference-2.17.0.pdf}
    \begin{itemize}
    \item[-] easiest to start from Part III Example Models
    \end{itemize}
...
\begin{frame}
  ...
  \item<3-> No U-Turn Sampling (NUTS) and dynamic HMC
    ...
    \item to keep reversibility of Markov chain, need to simulate on two direction
    \item \url{http://elevanth.org/blog/2017/11/28/build-a-better-markov-chain/}
    \end{itemize}
  \item<4-> Dynamic simulation is discretized
    \begin{itemize}
    \item small step size gives accurate simulation, but requires more log density evalutions
...
\begin{frame}
  ...
  \item Dynamic HMC using growing tree to increase simulation
    ...
    \item pick a draw along the trajectory with probabilities adjusted
      to take into account the error in the discreteized dynamic
      simulation
    \end{itemize}
  \item<2-> Mass matrix and step size adaptation in Stan
    \begin{itemize}
    ...
    \item mass matrix and step size adjustment and are estimated
      during initial adaptation phase
    \item step size is adjusted to be as big as possible while keeping
      discretization error in control
    \end{itemize}
  \item<3-> After adaptation the algorithm parameters are fixed and
    some further iterations included in the warmup
  \item<4-> After warmup store iterations for inference
  \item<5-> See more details in Stan reference manual
\end{itemize}
...
\begin{frame}
  ...
  \item Indicates inefficiency in sampling leading to higher
    autocorrelations and lower $n_{\rm eff}$
...
\begin{frame}
  ...
  \begin{itemize}
  \item HMC specific: Indicates that Hamiltonian dynamic simulation has problems
    going to narrow places
    \begin{itemize}
...
\begin{frame}
   ...
   \item<2-> Several full time developers, 35 in dev team, more than 100 contributors
   \item<3-> R, Python, Julia, Scala, Stata, Matlab, command line interfaces
    \item<3-> More than 100 R packages using Stan
   \end{itemize}
  \vfill
  \begin{center}
...
\end{lstlisting}}
  ...
  \item Stan makes transformation to unconstrained space and samples in unconstrained space
    ...
    \end{itemize}
  \item For these declared transformation Stan automatically takes
...
\end{lstlisting}}
    ...
    \begin{itemize}
    \item Stan model code constructs log density!
    \item<2-> lpdf for continuous, lpmf for discrete distributions
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  {\Large\color{navyblue} RStan and PyStan}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Stan compiles the model written in Stan language to C++
    \begin{itemize}
    \item this makes the sampling for complex models and bigger data faster
    \item also makes Stan models easily portable, you can use your own
      favorite interface
...
\begin{frame}
  ...
  \item Report divergences, $n_\eff$, $\hat{R}$
  \end{itemize}
...
\end{lstlisting}
  ...
  \begin{itemize}
  \item<2-> lp\_\_ is the log density, ie, same as ``target''
  \end{itemize}
...
\end{lstlisting}
  ...
  \begin{itemize}
  \item difference between ``vector[N] x'' and ``real x[N]''
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}
functions {
  real gpareto_log(vector y, real k, real sigma) {
...
\begin{frame}[fragile]
  ...
  \item R
    ...
    \item bayesplot --- visualization and model checking (se model checking in Ch 6)
