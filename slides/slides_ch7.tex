...
\begin{frame}{Pareto smoothed importance sampling LOO}
  ...
  \begin{list1}
  \item We want to compute ${\color{navyblue}p(y_i \mid x_{-i}, y_{-i})} = \int p(y_i \mid x_i,\theta){\color{blue} p(\theta \mid x_{-i}, y_{-i})} d\theta$
  \item Proposal distribution is full posterior $\theta^{(s)} \sim p(\theta \mid x,y)$
  \item Target distribution is LOO-posterior ${\color{blue} p(\theta \mid x_{-i}, y_{-i})}$
...
\begin{frame}[fragile]
  ...
  \item<+-> \texttt{rstan::loo(..., moment\_match = TRUE)}\\
    ...
    support moment matching algorithm by Paananen et al. (2021,
    Implicitly adaptive importance sampling), see also
    \url{http://mc-stan.org/loo/articles/loo2-moment-matching.html}
...
\begin{frame}{Sometimes cross-validation is not needed}
  ...
  \item With good priors that keep the prior on predictive space
    consistent, there is no need to do model selection to avoid
    overfitting
  \end{list1}
