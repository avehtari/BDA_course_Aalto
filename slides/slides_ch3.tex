...
\begin{frame}
  ...
  \begin{align*}
    ...
    \uncover<3->{p(\mu,\sigma^2 \mid y) & \propto  \sigma^{-n-2}\exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n(y_i-\mu)^2\right)}\\
    \uncover<4->{&  = \sigma^{-n-2}\exp\left(-\frac{1}{2\sigma^2}\left[\sum_{i=1}^n(y_i-\bar{y})^2+n(\bar{y}-\mu)^2\right]\right)}\\
    \uncover<4->{\color{gray} \text{where } \bar{y} & \color{gray} = \frac{1}{n}\sum_{i=1}^n y_i }\\
    \uncover<5->{&  = \sigma^{-n-2}\exp\left(-\frac{1}{2\sigma^2}\left[(n-1)s^2+n(\bar{y}-\mu)^2\right]\right)}\\
    \uncover<5->{\color{gray} \text{where }  s^2 & \color{gray} =\frac{1}{n-1}\sum_{i=1}^n(y_i-\bar{y})^2}
  \end{align*}
...
\begin{frame}
   ...
   {\large\color{navyblue} Marginal posterior $p(\sigma^2 \mid y)$ (easier for $\sigma^2$ than $\sigma$)}
    ...
    \uncover<4->{& &\hspace{2cm} \color{gray} \int \frac{1}{\sqrt{2\pi}\sigma} \exp\left(-\frac{1}{2\sigma^2}(y-\theta)^2\right) dy = 1}\\
    & \uncover<5->{\propto &
  ...
  \begin{minipage}[b][5cm][t]{5cm}
    ...
    \vspace{-0.5\baselineskip}
     \begin{align*}
       ...
       \uncover<3->{{\color{darkgreen} p(\mu \mid \sigma^2,y)} & = \N(\mu \mid \bar{y},\sigma^2/n)\,} \uncover<4>{ \color{gray} {\textstyle \propto \sigma^{-n-2}\exp\left(-\frac{1}{2\sigma^2}\left[(n-1)s^2+n(\bar{y}-\mu)^2\right]\right)}\\}
...
\begin{frame}
  ...
  {\color{navyblue} Marginal posterior $p(\mu \mid y)$}
    \begin{align*}
      ...
      \uncover<2->{& \propto \int_0^\infty \sigma^{-n-2}\exp\left(-\frac{1}{2\sigma^2}\left[(n-1)s^2+n(\bar{y}-\mu)^2\right]\right) d\sigma^2}
    \end{align*}
   \uncover<3->{Transformation\\}
    \vspace{-1\baselineskip}
    \begin{align*}
      \uncover<3->{A=(n-1)s^2+n(\mu-\bar{y})^2}\uncover<4->{\quad \text{and} \quad z=\frac{A}{2\sigma^2}}
    \end{align*}
%  \vspace{-\baselineskip}
    \begin{align*}
     \uncover<5->{p(\mu \mid y)&\propto A^{-n/2}\int_0^\infty z^{(n-2)/2}\exp(-z)dz}
   \uncover<6->{\intertext{\color{gray} Recognize gamma integral\, $\Gamma(u) = \int_0^\infty x^{u-1}\exp(-x)dx$}}
    \uncover<7->{&\propto [(n-1)s^2+n(\mu-\bar{y})^2]^{-n/2}\\}
    \uncover<8->{&\propto \left[1+\frac{n(\mu-\bar{y})^2}{(n-1)s^2}\right]\\}
...
\begin{frame}
  ...
  \vspace{-0.5\baselineskip}
    ...
    \begin{align*}
      ...
      \uncover<2->{\\ {\color{blue} \theta} & = \frac{1}{1+\exp({\color{darkgreen}\alpha+\beta x})}}
    \end{align*}
  \end{minipage}~~~
...
\begin{frame}
  ...
  \only<3-4>{\includegraphics[width=10cm]{bioassay_postld50.pdf}\\}
  \only<5>{\includegraphics[width=10cm]{bioassay_histld50.pdf}\\}
  \only<3->{
    \vspace{-\baselineskip}
        \begin{align*}
      \mbox{LD50:}\;\;\;
...
\begin{frame}
  ...
  \only<7>{\includegraphics[width=10cm]{bioassay_grid4.pdf}\\}
  \only<8>{\includegraphics[width=10cm]{bioassay_grid5.pdf}\\}
  \only<9>{\includegraphics[width=10cm]{bioassay_grid6.pdf}\\}
  \only<7-9>{
    \vspace{-0.5\baselineskip}
    \begin{itemize}
  ...
  \only<10>{\includegraphics[width=10cm]{bioassay_grid7.pdf}\\
    \vspace{-0.5\baselineskip}
    \begin{itemize}
